...
title XauMicro M5 - Installer
color 0F

set "SRC=%~dp0XauMicro_M5.mq5"
...
echo.
echo ================================================================
echo   XauMicro M5  -  MetaTrader 5 Installer
...
rem ---------------------------------------------------------------- source
if not exist "%SRC%" (
  echo   [ERROR] XauMicro_M5.mq5 غير موجود بنفس مجلد هذا الملف
  echo   [ERROR] XauMicro_M5.mq5 not found next to this installer
  echo.
...
echo   المصدر / Source : XauMicro_M5.mq5  ^(!SRCSIZE! bytes^)
if not "!SRCSIZE!"=="41750" (
  echo   [!] تحذير: الحجم المتوقع 41750 بايت. الملف قد يكون ناقصاً.
)
echo.
...
for /d %%T in ("%BASE%\*") do (
  if exist "%%~fT\MQL5\Experts" (
    ...
    copy /y "%SRC%" "!EXPERTS!\XauMicro_M5.mq5" >nul 2>&1
    if exist "!EXPERTS!\XauMicro_M5.mq5" (
      for %%F in ("!EXPERTS!\XauMicro_M5.mq5") do set "GOTSIZE=%%~zF"
    ...
    if defined MEPATH (
      ...
      "!MEPATH!" /compile:"!EXPERTS!\XauMicro_M5.mq5" /log:"!EXPERTS!\XauMicro_M5_compile.log" >nul 2>&1
      if exist "!EXPERTS!\XauMicro_M5.ex5" (
        set /a COMPILED+=1
        echo       [OK] تمت الترجمة بنجاح - XauMicro_M5.ex5 جاهز
      ) else (
        echo       [!] الترجمة لم تنتج ملف ex5. افتح MetaEditor واضغط F7.
        if exist "!EXPERTS!\XauMicro_M5_compile.log" echo           السجل: MQL5\Experts\XauMicro_M5_compile.log
      )
    ) else (
...
if %COMPILED% GTR 0 (
  ...
  echo      3^) راح يظهر XauMicro_M5 - اسحبه على شارت XAUUSD إطار M5
  ...
  echo      2^) بنافذة Navigator اليسرى: Experts ثم دبل-كليك XauMicro_M5
