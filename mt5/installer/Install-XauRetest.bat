...
title XauRetest M5 - Installer
color 0F

set "SRC=%~dp0XauRetest_M5.mq5"
...
echo.
echo ================================================================
echo   XauRetest M5  -  MetaTrader 5 Installer
...
rem ---------------------------------------------------------------- source
if not exist "%SRC%" (
  echo   [ERROR] XauRetest_M5.mq5 غير موجود بنفس مجلد هذا الملف
  echo   [ERROR] XauRetest_M5.mq5 not found next to this installer
  echo.
...
echo   المصدر / Source : XauRetest_M5.mq5  ^(!SRCSIZE! bytes^)
if not "!SRCSIZE!"=="60331" (
  echo   [!] تحذير: الحجم المتوقع 60331 بايت. الملف قد يكون ناقصاً.
)
echo.
...
for /d %%T in ("%BASE%\*") do (
  if exist "%%~fT\MQL5\Experts" (
    ...
    for %%D in (Experts Scripts Indicators) do (
      if exist "%%~fT\MQL5\%%D" (
        for %%N in (XauRetest_M5 GoldBot_M5 GoldBot aaa) do (
    ...
    copy /y "%SRC%" "!EXPERTS!\XauRetest_M5.mq5" >nul 2>&1
    if exist "!EXPERTS!\XauRetest_M5.mq5" (
      for %%F in ("!EXPERTS!\XauRetest_M5.mq5") do set "GOTSIZE=%%~zF"
    ...
    if defined MEPATH (
      ...
      "!MEPATH!" /compile:"!EXPERTS!\XauRetest_M5.mq5" /log:"!EXPERTS!\XauRetest_M5_compile.log" >nul 2>&1
      if exist "!EXPERTS!\XauRetest_M5.ex5" (
        set /a COMPILED+=1
        echo       [OK] تمت الترجمة بنجاح - XauRetest_M5.ex5 جاهز
      ) else (
        echo       [!] الترجمة لم تنتج ملف ex5. افتح MetaEditor واضغط F7.
        if exist "!EXPERTS!\XauRetest_M5_compile.log" echo           السجل: MQL5\Experts\XauRetest_M5_compile.log
      )
    ) else (
...
if %COMPILED% GTR 0 (
  ...
  echo      3^) راح يظهر XauRetest_M5 - اسحبه على شارت XAUUSD إطار M5
  ...
  echo      2^) بنافذة Navigator اليسرى: Experts ثم دبل-كليك XauRetest_M5
