...
title XauFlash - Installer
color 0F

set "SRC=%~dp0XauFlash_S.mq5"
...
echo.
echo ================================================================
echo   XauFlash  -  MetaTrader 5 Installer
...
rem ---------------------------------------------------------------- source
if not exist "%SRC%" (
  echo   [ERROR] XauFlash_S.mq5 غير موجود بنفس مجلد هذا الملف
  echo   [ERROR] XauFlash_S.mq5 not found next to this installer
  echo.
...
echo   المصدر / Source : XauFlash_S.mq5  ^(!SRCSIZE! bytes^)
if not "!SRCSIZE!"=="31134" (
  echo   [!] تحذير: الحجم المتوقع 31134 بايت. الملف قد يكون ناقصاً.
)
echo.
...
for /d %%T in ("%BASE%\*") do (
  if exist "%%~fT\MQL5\Experts" (
    ...
    copy /y "%SRC%" "!EXPERTS!\XauFlash_S.mq5" >nul 2>&1
    if exist "!EXPERTS!\XauFlash_S.mq5" (
      for %%F in ("!EXPERTS!\XauFlash_S.mq5") do set "GOTSIZE=%%~zF"
    ...
    if defined MEPATH (
      ...
      "!MEPATH!" /compile:"!EXPERTS!\XauFlash_S.mq5" /log:"!EXPERTS!\XauFlash_S_compile.log" >nul 2>&1
      if exist "!EXPERTS!\XauFlash_S.ex5" (
        set /a COMPILED+=1
        echo       [OK] تمت الترجمة بنجاح - XauFlash_S.ex5 جاهز
      ) else (
        echo       [!] الترجمة لم تنتج ملف ex5. افتح MetaEditor واضغط F7.
        if exist "!EXPERTS!\XauFlash_S_compile.log" echo           السجل: MQL5\Experts\XauFlash_S_compile.log
      )
    ) else (
...
if %COMPILED% GTR 0 (
  ...
  echo      3^) راح يظهر XauFlash_S - اسحبه على شارت XAUUSD أي إطار زمني
  ...
  echo      2^) بنافذة Navigator اليسرى: Experts ثم دبل-كليك XauFlash_S
