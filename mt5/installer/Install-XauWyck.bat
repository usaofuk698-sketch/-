...
title XauWyck M5 - Installer
color 0F

set "SRC=%~dp0XauWyck_M5.mq5"
...
echo.
echo ================================================================
echo   XauWyck M5  -  MetaTrader 5 Installer
...
rem ---------------------------------------------------------------- source
if not exist "%SRC%" (
  echo   [ERROR] XauWyck_M5.mq5 غير موجود بنفس مجلد هذا الملف
  echo   [ERROR] XauWyck_M5.mq5 not found next to this installer
  echo.
...
echo   المصدر / Source : XauWyck_M5.mq5  ^(!SRCSIZE! bytes^)
if not "!SRCSIZE!"=="42315" (
  echo   [!] تحذير: الحجم المتوقع 42315 بايت. الملف قد يكون ناقصاً.
)
echo.
...
for /d %%T in ("%BASE%\*") do (
  if exist "%%~fT\MQL5\Experts" (
    ...
    copy /y "%SRC%" "!EXPERTS!\XauWyck_M5.mq5" >nul 2>&1
    if exist "!EXPERTS!\XauWyck_M5.mq5" (
      for %%F in ("!EXPERTS!\XauWyck_M5.mq5") do set "GOTSIZE=%%~zF"
    ...
    if defined MEPATH (
      ...
      "!MEPATH!" /compile:"!EXPERTS!\XauWyck_M5.mq5" /log:"!EXPERTS!\XauWyck_M5_compile.log" >nul 2>&1
      if exist "!EXPERTS!\XauWyck_M5.ex5" (
        set /a COMPILED+=1
        echo       [OK] تمت الترجمة بنجاح - XauWyck_M5.ex5 جاهز
      ) else (
        echo       [!] الترجمة لم تنتج ملف ex5. افتح MetaEditor واضغط F7.
        if exist "!EXPERTS!\XauWyck_M5_compile.log" echo           السجل: MQL5\Experts\XauWyck_M5_compile.log
      )
    ) else (
...
if %COMPILED% GTR 0 (
  ...
  echo      3^) راح يظهر XauWyck_M5 - اسحبه على شارت XAUUSD إطار M5
  ...
  echo      2^) بنافذة Navigator اليسرى: Experts ثم دبل-كليك XauWyck_M5
