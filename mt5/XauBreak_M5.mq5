//+------------------------------------------------------------------+
//|                                                 XauBreak_M5.mq5  |
//|          Momentum-breakout scalper for spot gold on M5           |
//|                        XAUUSD - M5 - built for Exness MT5        |
//+------------------------------------------------------------------+
//| WHAT THIS IS
//| A momentum-breakout scalper, built around one piece of arithmetic. With a
//| 0.29 USD/oz round-trip cost, the hit rate needed merely to break even is
//| (stop + cost) / (target + stop):
//|
//|     target 1.00, stop 1.00  ->  64.5%   "take a quick dollar"
//|     target 4.48, stop 1.12  ->  25.2%   this EA
//|
//| Both keep the same tight stop. Only the exit differs, and it moves the
//| required hit rate by nearly forty points. On a spread-heavy instrument the
//| profitable direction is LARGER targets, not smaller ones -- the opposite of
//| what quick scalping suggests.
//|
//| So: enter fast on a decisive impulse, risk about a dollar, aim four. Most
//| breakouts fail, which is why the hit rate is designed to be low (~25% is
//| break-even, not a target). The few that work must reach the full distance.
//|
//| THE BREAK-EVEN TRAP
//| InpUseBreakeven defaults to false, and that is deliberate. Moving
//| the stop to break-even early feels prudent and is fatal here: the maths only
//| works if winners reach the target, and an early break-even stop turns a
//| large share of them into scratches. The time stop is the safety valve
//| instead -- a breakout that has not moved within a few bars has failed.
//|
...
input long   InpMagicNumber        = 770599; // Identifies this EA's own trades.
...
int OnInit()
  {
   ...
   PrintFormat("XauBreak M5 started on %s | server GMT offset %+d h | "
...
void OpenTrade(ENUM_ORDER_TYPE type, double price, double sl, double tp, double atrV)
  {
   ...
   bool ok = (type == ORDER_TYPE_BUY)
             ? trade.Buy(lots, _Symbol, 0.0, sl, tp, "XauBreak")
             : trade.Sell(lots, _Symbol, 0.0, sl, tp, "XauBreak");
