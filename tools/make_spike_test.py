"""Regenerate mt5/XauSpike_Test.mq5 from mt5/XauSpike2.mq5.
...
Run after every change to XauSpike2.mq5:  python3 tools/make_spike_test.py
"""
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
SRC = ROOT / "mt5" / "XauSpike2.mq5"
...
REPLACEMENTS = [
    ...
    ("//| WHAT THIS IS",
     ...
     "//|\n"
     "//| WHAT THIS IS"),
    ('#property copyright "XauSpike2"', '#property copyright "XauSpike_Test"'),
    ...
    ('   string comment = "XauSpike2 " + g_coreName[i];', '   string comment = "XauSpikeT " + g_coreName[i];'),
    ("   if(tester)\n      Print(\"TESTER",
     ...
     "\n"
     "   if(tester)\n      Print(\"TESTER"),
    ('"XauSpike2  |  %s', '"XauSpike TEST (fixed lots)  |  %s'),
]
